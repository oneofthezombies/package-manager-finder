...
class PackageJsonFinder implements Finder {
    ...
    findSync(options: FinderOptions): FindResult | null {
        ...
        const packageManagerName = this.parsePackageManagerValuePattern(packageManagerValue);
    ...
    async find(options: FinderOptions): Promise<FindResult | null> {
        const { cwd } = options;
        const jsonPath = this.jsonPath(cwd);
        if (!(await fs.promises.stat(jsonPath))) {
            return null;
        }
    ...
    private parsePackageManagerValueFromJson(content: string): string | undefined {
        const packageJson = JSON.parse(content);
        return packageJson.packageManager;
    }

    private parsePackageManagerValuePattern(value: string): PackageManagerName | undefined {
...
class LockfileFinder implements Finder {
    findSync(options: FinderOptions): FindResult | null {
        const { cwd } = options;
        for (const [packageManagerName, lockfileName] of Object.entries(lockfileNames)) {
